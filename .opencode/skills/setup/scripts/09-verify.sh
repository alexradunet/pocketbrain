...
if [ -f "$PROJECT_ROOT/.env" ]; then
  if grep -qE "^(OPENCODE_OAUTH_TOKEN|OPENCODE_API_KEY)=" "$PROJECT_ROOT/.env" 2>/dev/null; then
...
# 5. Check registered groups (in SQLite â€” the JSON file gets migrated away on startup)
REGISTERED_GROUPS=0
