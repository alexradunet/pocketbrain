...
src/container-runner.ts               container/agent-runner/
    â”‚                                      â”‚
    â”‚ spawns container                      â”‚ runs OpenCode SDK
    ...
    â”œâ”€â”€ data/sessions/{folder}/.opencode/ â”€â”€> /home/node/.opencode/ (isolated per-group)
...
| **OpenCode sessions** | `~/.opencode/projects/` | OpenCode CLI session history |
...
### 1. "OpenCode CLI process exited with code 1"
...
# OPENCODE_OAUTH_TOKEN=sk-ant-oat01-...  (subscription)
# OPENCODE_API_KEY=sk-ant-api03-...        (pay-per-use)
```
...
**Workaround:** The system extracts only authentication variables (`OPENCODE_OAUTH_TOKEN`, `OPENCODE_API_KEY`) from `.env` and mounts them for sourcing inside the container. Other env vars are not exposed.
...
echo '{}' | docker run -i \
  ...
  -c 'export $(cat /workspace/env-dir/env | xargs); echo "OAuth: ${#OPENCODE_OAUTH_TOKEN} chars, API: ${#OPENCODE_API_KEY} chars"'
```
...
```
/workspace/
â”œâ”€â”€ env-dir/env           # Environment file (OPENCODE_OAUTH_TOKEN or OPENCODE_API_KEY)
â”œâ”€â”€ group/                # Current group folder (cwd)
â”œâ”€â”€ project/              # Project root (main channel only)
â”œâ”€â”€ global/               # Global AGENTS.md (non-main only)
...
### 5. Session Not Resuming / "OpenCode CLI process exited with code 1"

If sessions aren't being resumed (new session ID every time), or OpenCode CLI exits with code 1 when resuming:
...
**Important:** `allowDangerouslySkipPermissions: true` is required when using `permissionMode: 'bypassPermissions'`. Without it, OpenCode CLI exits with code 1.
...
[ -f .env ] && (grep -q "OPENCODE_OAUTH_TOKEN=sk-" .env || grep -q "OPENCODE_API_KEY=sk-" .env) && echo "OK" || echo "MISSING - add OPENCODE_OAUTH_TOKEN or OPENCODE_API_KEY to .env"
