...
- **Pool bots** are send-only â€” each gets a Grammy `Api` instance (no polling)
...
Subagent calls send_message(text: "Found 3 results", sender: "Researcher")
  â†’ MCP writes IPC file with sender field
  â†’ Host IPC watcher picks it up
  â†’ Assigns pool bot #2 to "Researcher" (round-robin, stable per-group)
  â†’ Renames pool bot #2 to "Researcher" via setMyName
  â†’ Sends message via pool bot #2's Api instance
  â†’ Appears in Telegram from "Researcher" bot
```
...
1. **Update imports** â€” add `Api` to the Grammy import:
...
3. **Add pool functions** â€” place these before the `isTelegramConnected` function:
...
export async function sendPoolMessage(
  ...
  if (poolApis.length === 0) {
    // No pool bots â€” fall back to main bot
...
1. **Add imports** â€” add `sendPoolMessage` and `initBotPool` from the Telegram swarm module, and `TELEGRAM_BOT_POOL` from config.

2. **Update IPC message routing** â€” in `src/ipc.ts`, find where the `sendMessage` dependency is called to deliver IPC messages (inside `processIpcFiles`). The `sendMessage` is passed in via the `IpcDeps` parameter. Wrap it to route Telegram swarm messages through the bot pool:
...
Note: The assistant name prefix is handled by `formatOutbound()` in the router â€” Telegram channels have `prefixAssistantName = false` so no prefix is added for `tg:` JIDs.

3. **Initialize pool in `main()` in `src/index.ts`** â€” after creating the Telegram channel, add:
...
Create *exactly* the team the user asked for â€” same number of agents, same roles, same names. Do NOT add extra agents, rename roles, or use generic names like "Researcher 1". If the user says "a marine biologist, a physicist, and Alexander Hamilton", create exactly those three agents with those exact names.
...
3. Keep group messages *short* â€” 2-4 sentences max per message. Break longer content into multiple `send_message` calls. No walls of text.
4. Use the `sender` parameter consistently â€” always the same name so the bot identity stays stable.
5. NEVER use markdown formatting. Use ONLY WhatsApp/Telegram formatting: single *asterisks* for bold (NOT **double**), _underscores_ for italic, â€¢ for bullets, ```backticks``` for code. No ## headings, no [links](url), no **double asterisks**.
...
```bash
npm run build
./container/build.sh  # Required â€” MCP tool changed
...
- Pool bots use Grammy's `Api` class â€” lightweight, no polling, just send
- Bot names are set via `setMyName` â€” changes are global to the bot, not per-chat
- A 2-second delay after `setMyName` allows Telegram to propagate the name change before the first message
- Senderâ†’bot mapping is stable within a group (keyed as `{groupFolder}:{senderName}`)
- Mapping resets on service restart â€” pool bots get reassigned fresh
