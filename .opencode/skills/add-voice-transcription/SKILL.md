...
```bash
bun scripts/apply-skill.ts .opencode/skills/add-voice-transcription
```
...
- `modify/src/channels/whatsapp.ts.intent.md` â€” what changed and invariants for whatsapp.ts
- `modify/src/channels/whatsapp.test.ts.intent.md` â€” what changed for whatsapp.test.ts
...
Look for:
- `Transcribed voice message` â€” successful transcription with character count
- `OPENAI_API_KEY not set` â€” key missing from `.env`
- `OpenAI transcription failed` â€” API error (check key validity, billing)
- `Failed to download audio message` â€” media download issue
...
3. Check OpenAI billing â€” Whisper requires a funded account

### Voice notes show "[Voice Message - transcription failed]"

Check logs for the specific error. Common causes:
- Network timeout â€” transient, will work on next message
