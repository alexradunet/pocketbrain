...
>    - Go to **APIs & Services â†’ Credentials** (in the left sidebar)
...
```bash
bunx -y @gongrzhe/server-gmail-autoauth-mcp auth
```

If that doesn't work (some versions don't have an auth subcommand), run it and let it prompt:

```bash
timeout 60 bunx -y @gongrzhe/server-gmail-autoauth-mcp || true
```
...
```bash
echo '{"method": "tools/list"}' | timeout 10 bunx -y @gongrzhe/server-gmail-autoauth-mcp 2>/dev/null | head -20 || echo "MCP responded (check output above)"
```
...
```typescript
gmail: { command: 'bunx', args: ['-y', '@gongrzhe/server-gmail-autoauth-mcp'] }
```
...
Add this mount block (after the `.opencode` mount is a good location):
...
export async function checkForNewEmails(): Promise<EmailMessage[]> {
  ...
  // Option 1: Use @opencode-ai/sdk with just gmail MCP
...
bunx -y @gongrzhe/server-gmail-autoauth-mcp
```
