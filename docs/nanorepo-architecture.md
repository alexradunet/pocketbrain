...
2. **OpenCode CLI** — reads `SKILL.md`, `.intent.md`, and `state.yaml` to resolve conflicts git can't handle. Caches resolutions via `git rerere` so the same conflict never needs resolving twice.
3. **OpenCode CLI + user input** — when OpenCode CLI lacks sufficient context to determine intent (e.g., two features genuinely conflict at an application level), it asks the user for a decision, then uses that input to perform the resolution. OpenCode CLI still does the work — the user provides direction, not code.
...
Before any operation, all affected files are copied to `.pocketbrain/backup/`. On success, backup is deleted. On failure, backup is restored. Works safely for users who don't use git.

## The Shared Base

`.pocketbrain/base/` holds a clean copy of the core codebase. This is the single common ancestor for all three-way merges, only updated during core updates.
...
### Intent Files
Each modified file has a `.intent.md` with structured headings: **What this skill adds**, **Key sections**, **Invariants**, and **Must-keep sections**. These give OpenCode CLI specific guidance during conflict resolution.
...
6. Conflict resolution (shared cache → `git rerere` → OpenCode CLI → OpenCode CLI + user input)
...
`.pocketbrain/resolutions/` ships pre-computed, verified conflict resolutions with **hash enforcement** — a cached resolution only applies if base, current, and skill input hashes match exactly. This means most users never encounter unresolved conflicts for common skill combinations.
...
`.pocketbrain/state.yaml` records: core version, all applied skills (with per-file hashes for base/skill/merged), structured operation outcomes, custom patches, and path remaps. This makes drift detection instant and replay deterministic.
...
2. **Three-level resolution: git → OpenCode CLI → OpenCode CLI + user input.**
