# PocketBrain Skills Architecture

## Core Principle

Skills are self-contained, auditable packages that apply programmatically via standard git merge mechanics. OpenCode CLI orchestrates the process — running git commands, reading skill manifests, and stepping in only when git can't resolve a conflict on its own. The system uses existing git features (`merge-file`, `rerere`, `apply`) rather than custom merge infrastructure.
...
2. **OpenCode CLI** — reads `SKILL.md`, `.intent.md`, migration guides, and `state.yaml` to understand context. Resolves conflicts that git can't handle programmatically. Caches the resolution via `git rerere` so it never needs to resolve the same conflict again.
3. **User** — OpenCode CLI asks the user when it lacks context or intent. This happens when two features genuinely conflict at an application level (not just a text-level merge conflict) and a human decision is needed about desired behavior.
...
Before any operation, the system copies all files that will be modified to `.pocketbrain/backup/`. On success, the backup is deleted. On failure, the backup is restored. This provides rollback safety regardless of whether the user commits, pushes, or understands git.

---

## 1. The Shared Base

`.pocketbrain/base/` holds the clean core — the original codebase before any skills or customizations were applied. This is the stable common ancestor for all three-way merges, and it only changes on core updates.
...
- **bun version conflicts**: two skills request incompatible semver ranges for the same package
...
2. **Level 2 (OpenCode CLI)**: if automatic resolution fails, OpenCode proposes options based on skill intents
...
- Auditable: `diff .pocketbrain/base/src/server.ts skills/add-whatsapp/modify/src/server.ts` shows exactly what the skill changes
- Deterministic: same three inputs always produce the same merge result
- Size is negligible since PocketBrain's core files are small
...
Structured headings (What, Key sections, Invariants, Must-keep) give OpenCode CLI specific guidance during conflict resolution instead of requiring it to infer from unstructured text.
...
test: "bun test src/channels/whatsapp.test.ts"

# --- Future fields (not yet implemented in v0.1) ---
# author: pocketbrain-team
# license: MIT
...
2. OpenCode CLI asks if the user wants to make any modifications
3. User describes what they want different
4. OpenCode CLI makes the modifications on top of the freshly applied skill
...
applied_skills:
  - skill: telegram
    version: 1.0.0
    custom_patch: .pocketbrain/custom/telegram-group-only.patch
...
8. Apply structured operations (bun deps, env vars, docker-compose — batched)
9. Run `bun install` (once, if any structured bun_dependencies exist)
...
When a user runs the skill's slash command in OpenCode CLI:
...
Copy all files that will be modified to `.pocketbrain/backup/`. If the operation fails at any point, restore from backup.
...
```bash
git merge-file src/server.ts .pocketbrain/base/src/server.ts skills/add-whatsapp/modify/src/server.ts
```
...
1. **Check shared resolution cache** (`.pocketbrain/resolutions/`) — load into local `git rerere` if a verified resolution exists for this skill combination. **Only apply if input hashes match exactly** (base hash + current hash + skill modified hash).
2. **`git rerere`** — checks local cache. If found, applied automatically. Done.
3. **OpenCode CLI** — reads conflict markers + `SKILL.md` + `.intent.md` (Invariants, Must-keep sections) of current and previously applied skills. Resolves. `git rerere` caches the resolution.
4. **User** — if OpenCode CLI cannot determine intent, it asks the user for the desired behavior.
...
- Merge bun dependencies into `package.json` (check for version conflicts)
...
2. Update `.pocketbrain/state.yaml` — skill record, file hashes (base, skill, merged per file), structured outcomes
3. **Run skill tests** — mandatory, even if all merges were clean
4. If tests fail on a clean merge → escalate to Level 2 (OpenCode CLI diagnoses the semantic conflict)

### Step 9: Clean Up

If tests pass, delete `.pocketbrain/backup/`. The operation is complete.

If tests fail and Level 2 can't resolve, restore from `.pocketbrain/backup/` and report the failure.
...
`git rerere` is local by default. But PocketBrain has thousands of users applying the same skill combinations. Every user hitting the same conflict and waiting for OpenCode CLI to resolve it is wasteful.

### The Solution

PocketBrain maintains a verified resolution cache in `.pocketbrain/resolutions/` that ships with the project. This is the shared artifact — **not** `.git/rr-cache/`, which stays local.
...
git merge-file current.ts .pocketbrain/base/src/file.ts skills/add-whatsapp/modify/src/file.ts

# 2. If exit code > 0 (conflict), set up rerere adapter:

# Create blob objects for the three versions
base_hash=$(git hash-object -w .pocketbrain/base/src/file.ts)
...
`.pocketbrain/state.yaml` records everything about the installation:
  ...
  - name: telegram
    ...
    structured_outcomes:
      ...
      env_additions:
        - TELEGRAM_BOT_TOKEN
        - TELEGRAM_ONLY
      test: "bun test src/channels/telegram.test.ts"
  ...
  - name: discord
    ...
    structured_outcomes:
      ...
      env_additions:
        - DISCORD_BOT_TOKEN
        - DISCORD_ONLY
      test: "bun test src/channels/discord.test.ts"
...
custom_modifications:
  - description: "Added custom logging middleware"
    ...
    files_modified:
      - src/server.ts
    patch_file: .pocketbrain/custom/001-logging-middleware.patch
```
...
- `structured_outcomes` stores the raw manifest values plus the `test` command. Resolved bun versions (actual installed versions vs semver ranges) are not yet tracked.
...
2. **OpenCode CLI**: read `state.yaml` to understand what skills were applied, compare against actual file state, identify discrepancies
3. **User**: OpenCode CLI asks what they intended, what to keep, what to discard
...
Core updates must be as programmatic as possible. The PocketBrain team is responsible for ensuring updates apply cleanly to common skill combinations.
...
Three-way merge correctly keeps user's customizations, reverts the breaking change, and preserves all bug fixes. If there's a conflict, normal resolution: cache → OpenCode → user.
...
Copy all files that will be modified to `.pocketbrain/backup/`.
...
```bash
git merge-file src/server.ts .pocketbrain/base/src/server.ts updates/0.5.0-to-0.6.0/files/src/server.ts
```
...
3. OpenCode CLI with `migration.md` + skill intents → resolves
4. User → only for genuine ambiguity

#### Step 7: Re-apply Custom Patches

```bash
git apply --3way .pocketbrain/custom/001-logging-middleware.patch
```
...
`.pocketbrain/base/` replaced with new clean core. This is the **only time** the base changes.
...
If the user has a custom patch on a skill that changed significantly, the patch may conflict. Normal resolution: cache → OpenCode → user.
...
3. Backup the current codebase to `.pocketbrain/backup/`
...
2. Updates `.pocketbrain/base/` to the current core version's clean files
...
```bash
# Fully programmatic — no OpenCode CLI needed

# 1. Install core at specified version
pocketbrain-init --version 0.5.0

# 2. Load shared resolutions into local rerere cache
load-resolutions .pocketbrain/resolutions/
...
for skill in state.applied_skills:
  ...
  for file in skill.files_modified:
    ...
    git merge-file ${resolved_path} .pocketbrain/base/${resolved_path} skills/${skill.name}/modify/${file}
...
merge_bun_dependencies → write package.json once
bun install once
...
Ship with PocketBrain to reduce noisy merge conflicts:
...
project/
  src/                              # The actual codebase
    ...
    channels/
      whatsapp.ts
      telegram.ts
  skills/                           # Skill packages (OpenCode CLI slash commands)
    add-whatsapp/
      SKILL.md
      manifest.yaml
      tests/
...
5. **One shared base.** `.pocketbrain/base/` is the clean core before any skills or customizations. It's the stable common ancestor for all three-way merges. Only updated on core updates.
...
7. **Resolutions are learned and shared.** Maintainers resolve conflicts and ship verified resolutions with hash enforcement. `.pocketbrain/resolutions/` is the shared artifact.
