...
PocketBrain runs as a **single long-lived Bun process** inside a Docker container. The OpenCode SDK starts an embedded OpenCode server (port 4096) and manages per-chat agent sessions in-process. There are no per-invocation containers.
...
│  │  Per-chat sessions via client.session.*              │  │          │
...
| Message Storage | SQLite (`bun:sqlite`) | Store messages, sessions, tasks, chats |
| Agent Engine | `@opencode-ai/sdk` (`createOpencode`) | Embedded OpenCode server + per-chat sessions |
...
│
├── src/
│   ├── index.ts                   # Orchestrator: message loop, chat queue, agent invocation
...
│   ├── group-queue.ts             # Per-chat concurrency with MAX_CONCURRENT_SESSIONS
...
│   └── {chat-folder}/             # Per-chat folders (created on registration)
│       ├── AGENTS.md              # Chat-specific memory
...
│           └── current_tasks.json # Snapshot: scheduled tasks visible to this chat
│
...
# Tuning
MAX_CONCURRENT_SESSIONS=3              # Max parallel chat sessions
...
|-------|----------|---------|------------|---------|
| **Global** | `groups/global/AGENTS.md` | All chats (via OpenCode instructions) | Any chat | Preferences, facts shared across conversations |
| **Chat** | `groups/{name}/AGENTS.md` | That chat (injected at session start) | That chat | Chat-specific context, conversation memory |
| **Files** | `groups/{name}/*.md` | That chat | That chat | Notes, research, documents |
...
2. **Chat context**: At the start of each new session, `buildGroupContext()` reads `groups/{name}/AGENTS.md` and injects it into the first prompt, along with the session's `pocketbrain_context` (chatJid, groupFolder).

3. **Writing memory**: Agent writes to `./AGENTS.md` for chat memory, or uses the global file for cross-session facts.
...
1. Each registered chat has a session ID stored in SQLite (`sessions` table, keyed by `group_folder`)
2. On new message: `startSession()` creates a new session or resumes the existing one via `client.session.get()`
3. Follow-up messages from the same chat (while session is active) are sent via `sendFollowUp()`
...
5. Router checks:
   ...
   │
   ▼
6. GroupQueue serializes processing per chat:
...
When a message arrives, the agent receives all messages since its last interaction in that chat:

```
[Jan 31 2:32 PM] John: hey, should we do pizza tonight?
...
### Available in Any Registered Chat

| Action | Effect |
|--------|--------|
| Send any message | Agent responds |
| "list my scheduled tasks" | View this chat's tasks |
| "pause/resume/cancel task [id]" | Manage tasks |

---

## Scheduled Tasks

The scheduler runs tasks as full agent sessions in their chat's context.
...
| Tool | Purpose |
|------|---------|
| `send_message` | Send a message to the chat (via IPC → WhatsApp) |
| `schedule_task` | Schedule a recurring or one-time task |
| `list_tasks` | Show this chat's scheduled tasks |
...
The MCP tools write JSON files to `data/ipc/{chatFolder}/` using atomic temp-file-then-rename writes. The IPC watcher (`src/ipc.ts`) polls these directories and processes the commands on the host side.
