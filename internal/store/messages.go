...
func (r *MessageRepo) Append(sessionID, role, content string) error {
	return r.db.exec(func() error {
		return withStmt(r.db.conn, "INSERT INTO messages (session_id, role, content, created_at) VALUES (?, ?, ?, ?)", func(stmt *sqlite3.Stmt) error {
...
func (r *MessageRepo) GetBySession(sessionID string) ([]ChatMessage, error) {
	...
	err := r.db.exec(func() error {
		return withStmt(r.db.conn, "SELECT id, session_id, role, content, created_at FROM messages WHERE session_id = ? ORDER BY id", func(stmt *sqlite3.Stmt) error {
			stmt.BindText(1, sessionID)
			for stmt.Step() {
...
func (r *MessageRepo) GetRecentBySession(sessionID string, limit int) ([]ChatMessage, error) {
	var msgs []ChatMessage
	err := r.db.exec(func() error {
		return withStmt(r.db.conn, "SELECT id, session_id, role, content, created_at FROM messages WHERE session_id = ? ORDER BY id DESC LIMIT ?", func(stmt *sqlite3.Stmt) error {
			stmt.BindText(1, sessionID)
...
func (r *MessageRepo) DeleteBySession(sessionID string) error {
	return r.db.exec(func() error {
		return withStmt(r.db.conn, "DELETE FROM messages WHERE session_id = ?", func(stmt *sqlite3.Stmt) error {
			stmt.BindText(1, sessionID)
			stmt.Step()
			return stmt.Err()
		})
