...
func NewOutboxRepo(db *DB, defaultMaxRetries int) *OutboxRepo {
	...
	return &OutboxRepo{db: db, defaultMaxRetries: defaultMaxRetries}
}
...
func (r *OutboxRepo) Enqueue(channel, userID, text string, maxRetries int) error {
	...
	return r.db.exec(func() error {
		return withStmt(r.db.conn, "INSERT INTO outbox (channel, user_id, text, created_at, retry_count, max_retries, next_retry_at) VALUES (?, ?, ?, ?, 0, ?, NULL)", func(stmt *sqlite3.Stmt) error {
...
func (r *OutboxRepo) ListPending(channel string) ([]core.OutboxMessage, error) {
	...
	err := r.db.exec(func() error {
		return withStmt(r.db.conn, "SELECT id, channel, user_id, text, retry_count, max_retries, next_retry_at FROM outbox WHERE channel = ? AND (next_retry_at IS NULL OR next_retry_at <= ?) ORDER BY id", func(stmt *sqlite3.Stmt) error {
			stmt.BindText(1, channel)
...
func (r *OutboxRepo) Acknowledge(id int64) error {
	return r.db.exec(func() error {
		return withStmt(r.db.conn, "DELETE FROM outbox WHERE id = ?", func(stmt *sqlite3.Stmt) error {
			stmt.BindInt64(1, id)
			stmt.Step()
			return stmt.Err()
		})
...
func (r *OutboxRepo) MarkRetry(id int64, retryCount int, nextRetryAt string) error {
	return r.db.exec(func() error {
		return withStmt(r.db.conn, "UPDATE outbox SET retry_count = ?, next_retry_at = ? WHERE id = ?", func(stmt *sqlite3.Stmt) error {
