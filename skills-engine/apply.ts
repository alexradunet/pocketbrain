...
import { POCKETBRAIN_DIR } from './constants.js';
...
export async function applySkill(skillDir: string): Promise<ApplyResult> {
  ...
  for (const relPath of manifest.modifies) {
    ...
    const basePath = path.join(projectRoot, POCKETBRAIN_DIR, 'base', resolvedPath);
  ...
  try {
    ...
    for (const relPath of manifest.modifies) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `pocketbrain-merge-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
    ...
    if (manifest.structured?.bun_dependencies) {
      const pkgPath = path.join(projectRoot, 'package.json');
      mergeBunDependencies(pkgPath, manifest.structured.bun_dependencies);
    }
    ...
    // Run dependency install if dependencies were added.
    if (
      manifest.structured?.bun_dependencies &&
      Object.keys(manifest.structured.bun_dependencies).length > 0
    ) {
      runBunInstall();
    }
