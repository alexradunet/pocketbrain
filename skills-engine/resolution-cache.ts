...
import { POCKETBRAIN_DIR, RESOLUTIONS_DIR, SHIPPED_RESOLUTIONS_DIR } from './constants.js';
...
export function findResolutionDir(
  ...
  // Check shipped resolutions (.opencode/resolutions/) first, then project-level
...
export function loadResolutions(
  ...
  for (const { relPath, preimage, resolution } of pairs) {
    ...
    const basePath = path.join(projectRoot, POCKETBRAIN_DIR, 'base', relPath);
