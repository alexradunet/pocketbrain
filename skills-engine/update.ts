  ...
  try {
    ...
    for (const relPath of preview.filesChanged) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `pocketbrain-update-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
    ...
    const remapFile = path.join(newCorePath, '.pocketbrain-meta', 'path_remap.yaml');
    ...
    for (const skill of state.applied_skills) {
      ...
      if (outcomes.bun_dependencies) {
        Object.assign(allNpmDeps, outcomes.bun_dependencies as Record<string, string>);
        hasNpmDeps = true;
      }
