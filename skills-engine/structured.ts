...
export function areRangesCompatible(
  ...
  if (existing.startsWith('^') && requested.startsWith('^')) {
    ...
    // Same major â€” take the higher version
    const resolved =
  ...
  if (existing.startsWith('~') && requested.startsWith('~')) {
    ...
    // Same major.minor â€” take higher patch
    const resolved =
      compareVersionParts(eParts, rParts) >= 0 ? existing : requested;
    return { compatible: true, resolved };
  }
...
export function mergeBunDependencies(
...
export function runBunInstall(): void {
  // Backward-compatible function name. Uses Bun as the package manager.
  const lockPath = fs.existsSync('bun.lock')
    ? 'bun.lock'
    : fs.existsSync('bun.lockb')
      ? 'bun.lockb'
      : null;
  const cmd = lockPath ? 'bun install --frozen-lockfile' : 'bun install';
