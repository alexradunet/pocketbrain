...
export async function rebase(newBasePath?: string): Promise<RebaseResult> {
  ...
  try {
    ...
    const stateFilePath = path.join(projectRoot, POCKETBRAIN_DIR, 'state.yaml');
      ...
      if (newBasePath) {
        ...
        for (const relPath of trackedFiles) {
          ...
          const tmpSaved = path.join(
            os.tmpdir(),
            `pocketbrain-rebase-${crypto.randomUUID()}-${path.basename(relPath)}`,
          );
        ...
        if (mergeConflicts.length > 0) {
          // Return with backup pending for OpenCode CLI / user resolution
          return {
            success: false,
            patchFile: patchPath,
            filesInPatch,
