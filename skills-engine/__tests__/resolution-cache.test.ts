...
describe('resolution-cache', () => {
  ...
  it('saveResolution creates directory structure with files and meta', () => {
    ...
    const resDir = path.join(tmpDir, '.pocketbrain', 'resolutions', 'skill-a+skill-b');
  ...
  it('saveResolution writes file_hashes to meta.yaml', () => {
    ...
    const resDir = path.join(tmpDir, '.pocketbrain', 'resolutions', 'alpha+beta');
  ...
  it('findResolutionDir finds shipped resolutions in .opencode/resolutions', () => {
    const shippedDir = path.join(tmpDir, '.opencode', 'resolutions', 'alpha+beta');
    fs.mkdirSync(shippedDir, { recursive: true });
    fs.writeFileSync(path.join(shippedDir, 'meta.yaml'), 'skills: [alpha, beta]\n');

    const result = findResolutionDir(['alpha', 'beta'], tmpDir);
    expect(result).not.toBeNull();
    expect(result).toContain('.opencode/resolutions/alpha+beta');
  });

  it('findResolutionDir prefers shipped over project-level', () => {
    // Create both shipped and project-level
    const shippedDir = path.join(tmpDir, '.opencode', 'resolutions', 'a+b');
    fs.mkdirSync(shippedDir, { recursive: true });
    ...
    expect(result).toContain('.opencode/resolutions/a+b');
  });
  ...
  describe('loadResolutions hash verification', () => {
    ...
    function setupResolutionDir(fileHashes: Record<string, any>) {
      ...
      const resDir = path.join(tmpDir, '.opencode', 'resolutions', 'alpha+beta');
    ...
    function setupInputFiles() {
      // Create base file
      fs.mkdirSync(path.join(tmpDir, '.pocketbrain', 'base', 'src'), { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.pocketbrain', 'base', 'src', 'config.ts'), baseContent);
