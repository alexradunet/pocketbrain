...
describe('ci-matrix', () => {
  describe('computeOverlapMatrix', () => {
    it('detects overlap from shared modifies entries', () => {
      const skills: SkillOverlapInfo[] = [
        { name: 'telegram', modifies: ['src/config.ts', 'src/index.ts'], bunDependencies: [] },
        { name: 'discord', modifies: ['src/config.ts', 'src/router.ts'], bunDependencies: [] },
      ];
    ...
    it('returns no entry for non-overlapping skills', () => {
      const skills: SkillOverlapInfo[] = [
        { name: 'telegram', modifies: ['src/telegram.ts'], bunDependencies: ['grammy'] },
        { name: 'discord', modifies: ['src/discord.ts'], bunDependencies: ['discord.js'] },
      ];
    ...
    it('detects overlap from shared bun dependencies', () => {
      const skills: SkillOverlapInfo[] = [
        { name: 'skill-a', modifies: ['src/a.ts'], bunDependencies: ['lodash', 'zod'] },
        { name: 'skill-b', modifies: ['src/b.ts'], bunDependencies: ['zod', 'express'] },
      ];
      ...
      expect(matrix[0].reason).toContain('shared bun packages');
      expect(matrix[0].reason).toContain('zod');
    });

    it('reports both modifies and bun overlap in one entry', () => {
      const skills: SkillOverlapInfo[] = [
        { name: 'skill-a', modifies: ['src/config.ts'], bunDependencies: ['zod'] },
        { name: 'skill-b', modifies: ['src/config.ts'], bunDependencies: ['zod'] },
      ];
    ...
    it('handles three skills with pairwise overlaps', () => {
      const skills: SkillOverlapInfo[] = [
        { name: 'a', modifies: ['src/config.ts'], bunDependencies: [] },
        { name: 'b', modifies: ['src/config.ts', 'src/router.ts'], bunDependencies: [] },
        { name: 'c', modifies: ['src/router.ts'], bunDependencies: [] },
      ];
    ...
    it('returns empty array for single skill', () => {
      const skills: SkillOverlapInfo[] = [
        { name: 'only', modifies: ['src/config.ts'], bunDependencies: ['zod'] },
      ];
  ...
  describe('extractOverlapInfo', () => {
    it('extracts modifies and bun dependencies using dirName', () => {
      const manifest = makeManifest({
        skill: 'telegram',
        modifies: ['src/config.ts'],
        structured: {
          bun_dependencies: { grammy: '^1.0.0', zod: '^3.0.0' },
        },
      });
      ...
      expect(info.bunDependencies).toEqual(['grammy', 'zod']);
    });
    ...
    it('handles manifest without structured field', () => {
      ...
      expect(info.bunDependencies).toEqual([]);
    });

    it('handles structured without bun_dependencies', () => {
      const manifest = makeManifest({
        skill: 'env-only',
        modifies: [],
        structured: {
  ...
  describe('generateMatrix with real filesystem', () => {
    ...
    it('detects structured bun overlap from disk manifests', () => {
      const skillsDir = path.join(tmpDir, '.opencode', 'skills');
