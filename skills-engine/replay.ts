...
import { BASE_DIR, POCKETBRAIN_DIR } from './constants.js';
...
export async function replaySkills(
    ...
    try {
      ...
      for (const relPath of manifest.modifies) {
        ...
        const tmpCurrent = path.join(
          os.tmpdir(),
          `pocketbrain-replay-${crypto.randomUUID()}-${path.basename(relPath)}`,
        );
      ...
      if (manifest.structured?.bun_dependencies) {
        Object.assign(allNpmDeps, manifest.structured.bun_dependencies);
        hasNpmDeps = true;
      }
  ...
  if (hasNpmDeps) {
    ...
    mergeBunDependencies(pkgPath, allNpmDeps);
  }
  ...
  // 5. Run dependency install if any deps.
  if (hasNpmDeps) {
    try {
      runBunInstall();
    } catch {
      // Dependency install failure is non-fatal for replay.
    }
  }
