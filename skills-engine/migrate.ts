...
import { BASE_DIR, CUSTOM_DIR, POCKETBRAIN_DIR } from './constants.js';
import { initNanoclawDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initNanoclawDir();
  console.log('Skills system initialized. .pocketbrain/ directory created.');
}
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      const filesModified = [...diff.matchAll(/^diff -ruN .+ (.+)$/gm)]
        ...
        .filter((f) => !f.startsWith('.pocketbrain'));
      ...
      console.log(
        'Custom modifications captured in .pocketbrain/custom/migration.patch',
      );
    } else {
